...
public final class Interpreter {
  ...
  public func output(at index: Int) throws -> Tensor {
    ...
    guard let cTensor = TfLiteInterpreterGetOutputTensor(cInterpreter, Int32(index)),
          let bytes = TfLiteTensorData(UnsafeMutablePointer(mutating: cTensor)),
      let nameCString = TfLiteTensorName(UnsafeMutablePointer(mutating: cTensor))
    else {
      throw InterpreterError.invokeInterpreterRequired
    }
    guard let dataType = Tensor.DataType(type: TfLiteTensorType(UnsafeMutablePointer(mutating: cTensor))) else {
      throw InterpreterError.invalidTensorDataType
    }

    let name = String(cString: nameCString)
    let rank = TfLiteTensorNumDims(UnsafeMutablePointer(mutating: cTensor))
    let dimensions = (0..<rank).map { Int(TfLiteTensorDim(UnsafeMutablePointer(mutating: cTensor), $0)) }
    let shape = Tensor.Shape(dimensions)
    let byteCount = TfLiteTensorByteSize(UnsafeMutablePointer(mutating: cTensor))
    let data = Data(bytes: bytes, count: byteCount)
    let cQuantizationParams = TfLiteTensorQuantizationParams(UnsafeMutablePointer(mutating: cTensor))
    let scale = cQuantizationParams.scale
    let zeroPoint = Int(cQuantizationParams.zero_point)
    var quantizationParameters: QuantizationParameters? = nil
    if scale != 0.0 {
